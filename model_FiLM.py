...
class FiLMFPMConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, dialation=1):
        super().__init__()
        self.conv = nn.Conv3d(in_channels, out_channels, kernel_size=kernel_size, stride=stride, padding=padding, dilation=dialation)
...
class FPM(nn.Module):
    def __init__(self, in_channels, out_channels, h):
        ...
        self.conv1 = FiLMFPMConv(
            in_channels, out_channels,
            kernel_size=(h, 1, 1),
            stride=(h, 1, 1),
            padding=(0, 0, 0)
        )
        self.conv2 = FiLMFPMConv(
            in_channels, out_channels,
            kernel_size=(h, 3, 3),
            stride=(h, 1, 1),
            padding=(0, 1, 1)
        ...
        self.conv4 = FiLMFPMConv(
            out_channels * 3 + in_channels, out_channels,
            kernel_size=3,
            stride=1,
            padding=1
        )
    ...
    def forward(self, x, film):
        ...
        x1 = self.conv1(x, film)  # (N, 2, 128, 100, 100) -> (N, 16, 8, 100, 100)

        x2 = self.conv2(x, film)  # (N, 2, 128, 100, 100) -> (N, 16, 8, 100, 100)

        x3 = self.conv3(x, film)  # (N, 2, 128, 100, 100) -> (N, 16, 8, 100, 100)
        ...
        x = self.conv4(x, film)  # (N, 50, 8, 100, 100) -> (N, 16, 8, 100, 100)
...
class UNet(nn.Module):
    ...
    def get_feature(self, x, film):
        ...
        x3 = self.down2(x2, film)  # (N, 128, 50, 50) -> (N, 128, 25, 25)
        x4 = self.down3(x3, film)  # (N, 128, 25, 25) -> (N, 128, 12, 12)
        x5 = self.down4(x4, film)  # (N, 128, 12, 12) -> (N, 128, 6, 6)

        x = self.up1(x5, x4, film)  # (N, 128, 6, 6) -> (N, 128, 12, 12)
        
        x = self.up2(x, x3, film)  # (N, 128, 12, 12) -> (N, 128, 25, 25)
        x = self.up3(x, x2, film)  # (N, 128, 25, 25) -> (N, 128, 50, 50)
        feature = self.up4(x, x1, film)  # (N, 128, 50, 50) -> (N, 128, 100, 100)
        
        return feature 

    def forward(self, x, film):

        feature = self.get_feature(x, film)
...
class UNetAva(UNet):
    def __init__(self, in_channels, channels, n_classes, ava_classes, return_feature=False, dc_norms="NN"):
